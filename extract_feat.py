...
t0 = time()
for filename in sorted(os.listdir('UCF101_images/images/')):
    image_feature = np.empty(shape=[0,4096])
    for f in sorted(os.listdir('UCF101_images/images/'+filename)):
        input_tensor = []
        img = cv2.imread('UCF101_images/images/'+filename+"/"+f)
    ...
    scipy.io.savemat('UCF101_images/vgg16_relu6/'+filename,{'Feature':image_feature},do_compression = True)
    print("Writing to "+'UCF101_images/vgg16_relu6/'+filename+'.mat')
